var rl4chall = {
    1:{
        ...
        eff1(){
            var comp = getRl4ChallComp(1).min(this.maxComp().sub(1)).toNumber()
            switch(comp){
                case 0:
                    return n(2.5)
                case 1:
    ...
    2:{
        desp(){return `MC2: 您每排只能购买${format(this.eff1())}个塌缩升级 目标:${format(this.goal())}镜面碎片 奖励:维度间产量减益降低(不对镜面维度生效) (^${format(this.rew1().add(0.5))} 下一级: ^${format(this.rew1(getRl4ChallComp(2).add(1)).add(0.5))}) (已完成${format(getRl4ChallComp(2))}/${format(this.maxComp())}次)`},
        goal(){return ten.pow(player.rl4ChallComp[2].add(1).pow(0.5))},
        eff1(){
            var comp = getRl4ChallComp(2).min(this.maxComp().sub(1)).toNumber()
            switch(comp){
                case 0:
                    return n(3)
                case 1:
...
function enterRl4Chall(id){
    if(!confirm("您确定要进入挑战么?这会强制进行一次镜面重置,并且您不能从这次重置中获取镜面物质.这么做还会导致您的镜面碎片被清除.*镜面挑战中不能获得镜物质和镜面重置次数*")) return
