...
function getMSboost(MS = player.mirrorShard,show = true){
    if(hasRl4Milestone(8)) MS = MS.add(10)
    var power = MS.add(1).log10().add(1).pow(0.5)
...
function updatemd(){
    ...
    if(hasRl4Milestone(8)) player.md[3].num = player.md[3].num.max(player.md[0].num)
    ...
    w("mirrorize",`您镜面化了 ${format(player.mirrorizeTimes)} 次 这使得镜面维度倍率x${format(player.mirrorizeTimes.pow(3))}`)
