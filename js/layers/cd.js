...
function getCEEffExp(){
    var exp = n(0)
    if(hasUpgrade("cu",14)) exp = exp.add(cuEff(14))
    if(hasUpgrade("cu",24)) exp = exp.add(cuEff(24))
    if(!hasUpgrade("cu",14)) exp = n(0)
    return exp
}
function getCEEff(){
...
function updatecd(){
    //计算部分!
    var proc = n(1)
    if(!hasUpgrade("cu",14)) proc = n(0)
    if(hasUpgrade("cu",14)) player.cd[0].num = player.cd[0].num.max(1)
    cdMult.all = getAllcdMult()
    for(i in basicDimNums) cdMult[i] = getAnycdMult(i)
    for(i in basicDimNums){
        i = Number(i)
        if(i<=2) player.cd[i].num = player.cd[i].num.add(player.cd[i+1].num.mul(cdMult[i+1]).root(dimNerf).mul(diff).div(10))
        player.cd[i].procmult = player.cd[i].procmult.add(player.cd[i].num.mul(cdMult[i]).mul(diff))
        proc = proc.mul(player.cd[i].procmult)
    }
    player.ce = player.ce.add(proc.mul(diff))
