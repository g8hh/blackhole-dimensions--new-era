...
var TDcostMult = [n(10),n(100),n(1e8),n(1e10)]
function buyMaxtd(){
...
function updatetd(){
    ...
    for(i in basicDimNums){
        i = Number(i)
        if(i<=2) player.td[i].num = player.td[i].num.add(player.td[i+1].num.mul(tdMult[i+1]).root(dimNerf).mul(diff).mul(getBCTickspeed()).div(10))
        player.td[i].procmult = player.td[i].procmult.add(player.td[i].num.mul(tdMult[i]).mul(diff).mul(getBCTickspeed()))
        proc = proc.mul(player.td[i].procmult)
    }
    if(inRl3Chall(30)) proc = powsoftcap(proc,player.mass.pow(0.33),2)
    proc = sc("ts",proc)
    player.ts = player.ts.add(proc.mul(diff).mul(getBCTickspeed()))
    

    //显示部分!
    if(SecondTab!="时间维度") return close("td")
    open("td")
    w("ts",`您有 ${format(player.ts,0)} 时间碎片(+ ${format(proc)} /s) ( 效果指数: ${format(getTSEffExp(),3)} ) 这使得黑洞维度倍率x${format(getTSEff(0))}(对高维效果减弱)`)
    for(i in basicDimNums){
