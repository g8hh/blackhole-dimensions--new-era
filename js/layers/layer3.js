...
var cu = {
    11:{
        desp(){return "塌缩要求/e25.(Tip:任何降低塌缩要求的时候都会改善塌缩点获取公式)"},
        effect(){return n(1e25)},
        cost(){return n(2).pow(getCuCostPower(1)).floor()}
    },
    12:{
        desp(){return "物质上限^1.5.(Tip:您真的确定您能在无限后成吨的软上限中达到上限么)"},
        effect(){return n(1.5)},
        cost(){return n(2).pow(getCuCostPower(1)).floor()}
    },
    13:{
...
function resetCU(){
    if(confirm("您确定重置塌缩升级吗?(第1/3次确认)这不会返还任何塌缩点数!(您的塌缩点若低于2则使您的塌缩点等于2)")){
...
function doRl3(){
    if(player.mass.lt(getRl3Req())) return
