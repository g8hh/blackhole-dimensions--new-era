...
function format(decimal, precision = 2, small=false) {
    ...
    let part1 = "e".repeat(egg(decimal.array[1])+1 - (decimal.gte(OmegaNum.E_MAX_SAFE_INTEGER)))
    ...
    let part1 = "F".repeat(egg(decimal.array[2])+1 - (decimal.gte(OmegaNum.TETRATED_MAX_SAFE_INTEGER)))
    if(part1 != "F") 
    {
      decimal.array.pop()
      return part1+format(decimal)
